...
public class CurrencyExchangeService {
	private HashMap<String, BigDecimal> exchangeRate;

	public CurrencyExchangeService() {
		exchangeRate = new HashMap<>();

		exchangeRate.put("USD", BigDecimal.ONE);
		exchangeRate.put("KRW", BigDecimal.ONE.divide(BigDecimal.valueOf(1472), RoundingMode.HALF_EVEN));
	}
	...
	private BigDecimal getExchangeRate(String fromCurrency, String toCurrency) {
		...
		String from = fromCurrency.trim().toLowerCase().substring(0, 3);
		String to = toCurrency.trim().toLowerCase().substring(0, 3);
